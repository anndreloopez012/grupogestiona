...
if (isset($_GET["validaciones"]) && !empty($_GET["validaciones"])) {
    ...
    if ($strTipoValidacion == "insert_home") {
        ...
        print json_encode($arrInfo);
        die();
    } else if ($strTipoValidacion == "insert_empresas") {
        header('Content-Type: application/json');
        ...
        $var_consulta = "INSERT INTO empresas (nombre,img,descrip,url) VALUES ('$nombre','$img','$descrip','$url');";
        $val = 1;
        if (mysqli_query($link, $var_consulta)) {
            $arrInfo['status'] = $val;
        } else {
        ...
        print json_encode($arrInfo);
        die();
    } else if ($strTipoValidacion == "delete_empresas") {
        header('Content-Type: application/json');
        $idEmpresa = isset($_POST["idEmpresa"]) ? $_POST["idEmpresa"]  : ' ';
        $val = 1;
        $var_consulta = "DELETE FROM empresas WHERE id = $idEmpresa;";
        if (mysqli_query($link, $var_consulta)) {
            $arrInfo['status'] = $val;
        } else {
            $arrInfo['status'] = 0;
...
?>
    ...
    <?php
        die();
    } else if ($strTipoValidacion == "dibujo_tabla_empresas") {

        $arrEmpresa = array();
        $var_consulta = "SELECT * FROM empresas";

        //$var_resultado = $link->query($var_consulta);
        $qTMP = mysqli_query($link, $var_consulta);
        while ($rTMP = mysqli_fetch_assoc($qTMP)) {
            $arrEmpresa[$rTMP["id"]]["id"] = $rTMP["id"];
            $arrEmpresa[$rTMP["id"]]["nombre"] = $rTMP["nombre"];
            $arrEmpresa[$rTMP["id"]]["descrip"] = $rTMP["descrip"];
        }
        mysqli_free_result($qTMP);
    ?>
        <table class="table">
            ...
            <tbody>
                ...
                if (is_array($arrEmpresa) && (count($arrEmpresa) > 0)) {
                    reset($arrEmpresa);
                    $intContador = 1;
                    foreach ($arrEmpresa as $rTMP['key'] => $rTMP['value']) {
                ?>
                        <tr>
                            <th><?php echo  $rTMP["value"]['nombre']; ?></th>
                            <th><?php echo  $rTMP["value"]['descrip']; ?></th>
                            <th class="table-info" style="width:5%">
                                <i class="fad fa-user-minus " style="cursor:pointer;" id="delete" onclick="fntSelectDeleteEmpresas('<?php print $intContador; ?>'); "></i>
                            </th>
                        </tr>
