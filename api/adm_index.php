...
while ($rTMP = mysqli_fetch_assoc($qTMP)) {
    ...
    $arrServiceCont[$rTMP["id"]]["img_service"] = $rTMP["img_service"];
}
mysqli_free_result($qTMP);
...
if (is_array($arrServiceCont) && (count($arrServiceCont) > 0)) {
    ...
    foreach ($arrServiceCont as $rTMP['key'] => $rTMP['value']) { 
            ...
            $img_service = $rTMP["value"]['img_service'];
    }
}
...
//////////////////// QUESTION //////////////////////////////////////////
$arrQuestion = array();
$var_consulta = "SELECT * FROM request";

//$var_resultado = $conexion->query($var_consulta);
$qTMP = mysqli_query($link, $var_consulta);
while ($rTMP = mysqli_fetch_assoc($qTMP)) {
    $arrQuestion[$rTMP["id"]]["id"] = $rTMP["id"];
    $arrQuestion[$rTMP["id"]]["question"] = $rTMP["question"];
    $arrQuestion[$rTMP["id"]]["request"] = $rTMP["request"];
}
mysqli_free_result($qTMP);
...
$qTMP = mysqli_query($link, $var_consulta);
while ($rTMP = mysqli_fetch_assoc($qTMP)) {
    $arrEmp[$rTMP["id"]]["id"] = $rTMP["id"];
    $arrEmp[$rTMP["id"]]["nombre"] = $rTMP["nombre"];
    $arrEmp[$rTMP["id"]]["img"] = $rTMP["img"];
    $arrEmp[$rTMP["id"]]["descrip"] = $rTMP["descrip"];
}
mysqli_free_result($qTMP);
